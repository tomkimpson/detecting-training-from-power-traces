...
against a generator-based optimality ceiling. A parallel corpus attacks the
...
remains. This is the literature that would turn our empirical frontier into a
...
hiding rather than an upper bound on what our particular detector misses. This is
the identifiability question of \cref{sec:threat}, and the existing results do not
cover our object---a physics-constrained training schedule observed through a
filtered, low-rate meter---so such a bound, if it holds, is not inherited but must
be derived; we leave it as the central open direction.
...
tail was $3$--$6\times$ conservative. On the tested stationary nulls the
realised rate covers nominal at both the fixed-$\alpha$ and the fully adaptive
stage (white $0.047/0.009$, AR(1) $0.057/0.014$ at nominal $0.05/10^{-2}$
fixed-$\alpha$; white $0.050/0.006$, AR(1) $0.049/0.006$ fully adaptive), and
Kolmogorov--Smirnov uniformity is not rejected --- the property the analytic
$p$-values never achieved.\footnote{These surrogate numbers rest on a partial
grid ($M=1000$ null traces, $S=199$ surrogates, stages 1 and 4 only) and do not
yet cover the stationary non-Gaussian nulls or the held-out-warp stage. A
confirming run at $S=999$, $M=10^4$ including those cells is pending on a
compute cluster before any surrogate-calibrated number is frozen; the level-safety
and bake-off numbers above are already at $M=10^4$.} This is what
``calibrated'' means throughout the paper: an effective score with per-trace
surrogate calibration, not an analytic guarantee.
...
the surrogate-calibrated rate rises to $0.17$--$0.29$ at nominal $0.05$. This is
