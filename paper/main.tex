...
cost-of-hiding claim needs; extending it to the order-tracking class remains
open.
...
\begin{proposition}[No passive semantic identification]
	...
	with the same probability under both. Consequently no passive scalar-power detector can certify the semantic
	content of a computation against an unrestricted adversary: a guarantee
	requires restricting at least one of the admissible non-training workload
	class, the adversary's freedom to add or restructure computation, the
	hardware and power-transfer family, the physical property being detected, or
	the protocol.
\end{proposition}
...
\begin{proposition}[Necessary distortion for covertness]
	...
	$J(\sigma^*) = \varepsilon$ and diverges as
	$\varepsilon^{-1/2}$ when $\varepsilon \to 0$. Enlarging the verifier class can
	only raise $\sigma^*(\varepsilon)$.
\end{proposition}
...
\begin{remark}[Which inequality is doing the work]
	...
	$J$ certifies that smaller distortions fail. The price is that the guarantee
	is stated against the verifier class we exhibit: we prove it for the
	fixed-bin test, and we do \emph{not} prove it against the order-tracking
	class of \cref{sec:structural}, which our own experiments show to be
	strictly stronger.
\end{remark}

Distortion is not yet cost. The bridge is a physical property of the admissible
family in \cref{def:family}.

\begin{proposition}[Cost of hiding]
	\label{prop:cost}
	Within \cref{def:family}, reaching $\sigma^*(\varepsilon)$ costs throughput or
	learning efficiency; it cannot be free in both. Period jitter has
	only two realisations. Inserting idle time inflates the mean period and pays
	in throughput. Varying the real work per iteration avoids idling, but the
	down-phase duration is fixed by \cref{def:family} and the accumulated work is
	quantised, so a residual marker survives at the exchange cadence; removing it
	requires either idling or desynchronising the exchange, and the latter
	changes the optimisation that is being performed.
\end{proposition}

\begin{remark}[Scope of the bound]
	\label{rem:boundscope}
	Three limits fix the wording. The cost in \cref{prop:cost} is a disjunction,
...
overhead and never improves much on that however much the prover spends: the
cheapest attack is also close to the best one. Against the tracking class the
...
verifier (\cref{prop:sigmastar,prop:cost}), and the distortion it requires can be
priced against the same anchors used here. Closing it for the tracking class
remains open.
...
physics \cref{prop:cost} leans on. And an
...
\begin{table*}[t]
	...
	\begin{tabular}{@{}p{2.4cm}p{5.0cm}p{2.2cm}p{4.6cm}@{}}
		...
		loads with a training-shaped schedule score as training --- and
		\cref{prop:noident} says no passive detector can do better \\
		...
		Cost of hiding & Hiding from a fixed verifier to detectability
		$\varepsilon$ requires distortion $\sigma^*(\varepsilon)$, which costs
		throughput or learning efficiency and cannot cost neither (i.i.d.\ period
		jitter) & \cref{subsec:identifiability}, \cref{app:identifiability} &
		Proved against a fixed verifier only, for one attack family, and
		internal to the stated generator \\
...
and is proved against a fixed-bin verifier, not against the order tracker that
our own experiments show to be stronger, so it does not yet close the frontier's
far end. And the learning-efficiency leg of that bound --- the axis that would
...
Fractional period jitter perturbs each iteration boundary relative to its
predecessor, so the boundary phase $\theta_i = 2\pi i$ accumulates independent
increments and its residual against a uniform clock is a random walk with
$\mathrm{Var}[\theta(t)] \approx D t$, $D \approx (2\pi)^2\sigma^2 f_0$. For a
Brownian phase the periodic component's autocorrelation decays as
$\exp(-D|\tau|/2)$, so its spectrum is a Lorentzian of half-width proportional to
$D$ rather than a line. A record of length $T$ resolves frequency only to $1/T$,
so the coherent power a fixed-bin verifier collects is the fraction of that
Lorentzian falling inside one bin, which for a half-width small compared with the
band is $(1+\kappa\sigma^2)^{-1}$ with $\kappa \propto D T / \sigma^2 \propto
f_0 T$. The constant of proportionality depends on the width convention
(half-width at half-maximum versus equivalent noise bandwidth) and on the taper,
so the derivation fixes $\kappa$ only up to an $O(1)$ factor; the numerical check
below supplies it.
...
c]\right)$, the two-sample Kolmogorov--Smirnov distance of the score laws. Each
threshold test is a measurable function of the trace, so by data processing
...
$\varepsilon$-covertness requires $J(\sigma) \leq \varepsilon$. The statistic is
monotone in the coherent power it measures, which decays in $\sigma$ by
\cref{prop:rolloff}, so $J$ decays too and the requirement is a lower bound
$\sigma \geq \sigma^*(\varepsilon)$. Fitting the same functional form to $J$,
$J(\sigma) = J_0/(1+\kappa_J\sigma^2)$, and inverting gives
\begin{equation}
	\sigma^*(\varepsilon)
	\;=\; \kappa_J^{-1/2}\,\sqrt{J_0/\varepsilon - 1}
	\;\sim\; \left(\kappa_J\,\varepsilon/J_0\right)^{-1/2}
	\quad (\varepsilon \to 0).
	\label{eq:sigmastar}
\end{equation}
Enlarging the verifier class can only increase the attainable advantage at each
$\sigma$, hence only raise $\sigma^*$.

Two points of bookkeeping. The rolloff constant $\kappa_J$ of the
...
remains separable from a line-free null, so it decays far more slowly, and using
it is the conservative choice --- it yields the smaller, and therefore safely
necessary, $\sigma^*$. And $J$ is not interchangeable with the rank-based Gini
...
\paragraph{Argument for \cref{prop:cost}.}
The two realisations of period jitter are exhaustive, because a period is
occupied either by useful work or by idling. Padding the period with idle time
holds the real work per iteration fixed, so every period must be at least as long
as the compute it contains and the mean period inflates with $\sigma$; throughput
falls, and \cref{sec:frontier} measures the penalty at $15$--$375\%$ over the
swept range. Varying the real work per iteration avoids idling and is measured at
essentially zero throughput cost, but it cannot drive the cadence marker to zero
within \cref{def:family}. The accumulated work is an integer number of
micro-steps, so the up-phase is quantised rather than continuously tunable, and
the exchange down-phase keeps its duration whatever that count. A component at
the exchange cadence therefore survives every reallocation of work between
iterations. Removing it requires either inserting idle time, which returns to the
first realisation and its throughput cost, or desynchronising the exchange, which
alters the optimisation actually performed and so is paid for in learning
efficiency. The numerical check supports this rather than assuming it: at matched
$\sigma$ the work-varying attack is \emph{more} detectable than idle-padded
jitter, so the cheap-throughput route does not evade the bound for free --- it
trades throughput cost for detectability.
...
inference-null populations ($n=80$ per class, $f_0 = 1$~Hz, $T = 300$~s,
$f_s = 20$~Hz, seeded) are pushed through the full observation map and scored with
the fixed-bin statistic. The coherent power follows the form of
\cref{eq:rolloff} with $R^2 = 0.97$; the fitted $\kappa = 2948$ sits within a
factor of about three of the parameter-free $\pi f_0 T = 942$, which confirms the
functional form and fixes the $O(1)$ prefactor the derivation leaves open. The
attained advantage rolls off with $\kappa_J = 34$ at $R^2 = 0.95$ from
$J_0 = 1.0$, giving $\sigma^*(0.5) = 0.17$ and $\sigma^*(0.2) = 0.34$ inside the
swept range, and $\sigma^*(0.1) = 0.51$, $\sigma^*(0.05) = 0.74$ by
extrapolation. The generator's de-periodicisation anchors hold where the
in-range thresholds live: measured cadence coefficient of variation tracks
$\sigma$ and measured $D$ tracks $(2\pi)^2\sigma^2 f_0$ up to
$\sigma \approx 0.2$. Both break down by $\sigma \approx 0.35$, where a period
clamp in the generator takes over, so the extrapolated thresholds lie outside the
regime in which the jitter model itself is faithful and should be read as
indicative only. Finally, the work-varying series attains a higher advantage than
period jitter at every $\sigma \geq 0.1$, the comparison
\cref{prop:cost} rests on.

\begin{figure}[t]
	\centering
	\includegraphics[width=\linewidth]{lower_bound_spike}
	\caption{Numerical check on the covertness cost bound, through the full
	generator and meter. \textbf{Left:} coherent power at the nominal cadence
	against jitter scale, with the analytic and fitted rolloffs of
	\cref{eq:rolloff}. \textbf{Right:} the advantage attained by the fixed-bin
	threshold test, $J \leq \mathrm{TV}$, for idle-padded period jitter and for
	the near-free work-varying attack; dotted lines mark
	$\varepsilon \in \{0.1, 0.5\}$, whose crossings give
	$\sigma^*(\varepsilon)$. The work-varying attack is the more detectable of
	the two at matched distortion, which is the residual
	\cref{prop:cost} relies on. The verifier here is fixed at the known cadence,
	not the order tracker of \cref{sec:structural}.}
...
\cref{sec:frontier}, which price the idle-padded realisation at the same
$\sigma$ grid. Hiding from the fixed-bin verifier to $\varepsilon = 0.5$ needs
$\sigma^* = 0.17$, between the measured levels $\sigma = 0.1$ and $\sigma = 0.2$
and so between $32\%$ and $66\%$ throughput overhead; to $\varepsilon = 0.2$ it
needs $\sigma^* = 0.34$, between $66\%$ and $159\%$. This is the sense in which
the bound has a price, and \cref{fig:st2_cost_pareto} is its empirical
counterpart. The two must not be conflated: the thresholds come from a fixed-bin
verifier at the known cadence on $80$ traces per class, whereas the frontier
scores a five-detector bank on $200$, so they are read side by side rather than
plotted on shared axes.
