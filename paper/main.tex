...
\begin{remark}[Which inequality is doing the work]
	...
	test is ruled out against \emph{every} verifier class, the order tracker of
	\cref{sec:structural} included: the bound is universally valid. What it is not
...
\begin{remark}[Why this is an assumption]
	...
	the comparative one --- work variation is the more detectable of the two at
	matched distortion (\cref{fig:lower_bound}).
\end{remark}
...
Because any single verifier's advantage is bounded by the total variation, that
threshold is valid against the tracking class too --- but it is read off the weaker
detector, so it understates what a tracker would require. Tightening it there
remains open.
...
\begin{table*}[t]
	...
	\begin{tabular}{@{}p{2.4cm}p{5.0cm}p{2.2cm}p{4.6cm}@{}}
		...
		fixed-bin verifier, so valid everywhere but loose; one attack family;
...
and while it holds against every verifier class, it is obtained from a fixed-bin
test and so reads loose against the order tracker our own experiments show to be
stronger --- it does not yet close the frontier's far end. Its cost leg is an
...
conventions cancel once \cref{eq:arctan} is carried through explicitly. The
Lorentzian form of \cref{eq:rolloff} is itself an approximation to
\cref{eq:arctan}, accurate to better than $1\%$ over the range swept below.
...
$\sigma$ and for every $\sigma \geq 0.1$ the work-varying attack is \emph{more}
...
$R^2 = 0.95$ and a fitted $\kappa = 3020$ against the parameter-free
$\pi^2 f_0 T = 2961$ of \cref{eq:kappa} --- agreement to $2\%$ with nothing
fitted, which is the strongest single confirmation of the physical picture here.
...
$\sigma \in [0.05, 0.35]$ the advantage rolls off with $\kappa_J = 28$
($95\%$ bootstrap interval $[23, 37]$, $R^2 = 0.92$, $J_0 = 1.0$), giving
\begin{equation*}
	\sigma^*(0.5) = 0.19\ [0.17, 0.21], \qquad
	\sigma^*(0.2) = 0.38\ [0.33, 0.42],
\end{equation*}
and, by extrapolation beyond the swept range, $\sigma^*(0.1) = 0.57$ and
$\sigma^*(0.05) = 0.82$.
...
of variation runs high ($0.50$ at $\sigma = 0.35$, $1.48$ at $\sigma = 0.5$),
...
flags them. Second, the fit window necessarily reaches past $\sigma = 0.2$, since
the advantage is still saturated below it; restricting the fit to the faithful
region alone gives $\kappa_J = 19$ and hence larger thresholds, $\sigma^*(0.5) =
0.23$ and $\sigma^*(0.2) = 0.46$. The thresholds are therefore good to about
$20\%$, not to two significant figures, and the direction of that residual
...
$\sigma \geq 0.1$ --- and a lower one below that --- which is the comparison
\cref{ass:cost} appeals to.
...
\begin{figure}[t]
	...
	scale, agreeing to $2\%$. \textbf{Right:} the advantage attained by the
	...
	and is excluded from the fit. For $\sigma \geq 0.1$ the work-varying attack is
	the more detectable of the two at matched distortion, the comparison
	\cref{ass:cost} appeals to; below that it is the less detectable. The verifier
...
$\sigma^* = 0.19$, which falls between the measured levels $\sigma = 0.1$ and
$\sigma = 0.2$ and so between $32\%$ and $66\%$ throughput overhead; to
$\varepsilon = 0.2$ it needs $\sigma^* = 0.38$, between $159\%$ and $375\%$.
